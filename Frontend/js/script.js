...
async function fetchDashboardData() {
  ...
  try {
    ...
    // Update Cards
    getElement("totalSpent").innerText =
      money(data.totalExpense);

    getElement("totalGST").innerText =
      money(data.totalTax);
    ...
    getElement("topCategory").innerText =
      topCategory;
...
function renderDashboardCharts(data) {
  ...
  new Chart(bar, {
    ...
    data: {
      ...
      labels: Object.keys(data.merchantBreakdown),

      datasets: [{

        label: "Expenses",

        data: Object.values(data.merchantBreakdown),
...
async function uploadInvoice() {
  ...
  if (!fileInput.files[0]) {
    ...
    alert("Please choose invoice file");
